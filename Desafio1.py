...
df = pd.read_csv('MatPlotLib/Arquivos/titanic.csv', sep = ",")
...
plt.plot(df.SibSp.value_counts().sort_index(), color = 'gold', marker = 'o', label = 'Número de irmãos/conjuges à bordo')
plt.plot(df.Parch.value_counts().sort_index(), color = 'limegreen', marker = 'o', label = 'Número de filhos/pais à bordo')
plt.plot(nova_coluna.value_counts().sort_index(), color = 'royalblue', marker = 'o', label = 'Total de parentes à bordo')
plt.legend()
